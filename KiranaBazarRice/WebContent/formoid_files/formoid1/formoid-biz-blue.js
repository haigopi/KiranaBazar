...
jQuery.validator.addMethod("pattern", function(b, a, c) {
    return this.optional(a) || (new RegExp(/^\d{10}$/)).test(b)
}, jQuery.format("Please enter 10 digit number"));

jQuery.validator.origRequired=jQuery.validator.methods.required;jQuery.validator.addMethod("required",function(b,a,c){if(/^url$/.test(a.getAttribute("type"))&&($.trim(b)=="http://")){return false}return jQuery.validator.origRequired.call(this,b,a,c)});jQuery.validator.addMethod("name",function(b,a,c){if(/^name\[first\]|name\[last\]$/.test(a.getAttribute("name"))){return this.optional(a)||(new RegExp("^[^0-9\\<\\>\\\\/\\|*\\=\\_\\+\\~\\!\\?\\@\\#\\№\\$\\%\\^\\&\\(\\)\\{\\}\\[\\]\\;\\:\\`\\,]+$")).test(b)}return true},jQuery.format("Please enter the correct characters"));jQuery(function(){var a=jQuery;form=a(".formoid-biz-blue");form.validate({errorClass:"error-label",wrapper:"label",errorElement:"span",ignore:"",success:function(b,c){if(b.parent().hasClass("tooltip")){b.parent().remove()}else{b.remove()}var d=a(c).parent();d.removeClass("error-field")},errorPlacement:function(b,c){var d=a(c).parent();d.addClass("error-field");b.css("marginLeft",0).appendTo(d||c).addClass("tooltip").addClass("bottom").addClass("error").append(a("<span>").addClass("tooltip-arrow"))}});jQuery("[required=required]").keyup(function(){a(this).valid()})});